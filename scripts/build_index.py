...
# ---------------------------------------------------------------------------
# External enrichment: EPSS + CISA KEV
# ---------------------------------------------------------------------------

def fetch_epss():
    """FIRST EPSS daily scores -> {cve: {'score': float, 'percentile': float}}.

    CSV format after two header lines (#model_version..., then 'cve,epss,percentile').
    """
    req = urllib.request.Request(EPSS_URL, headers={"User-Agent": USER_AGENT})
    out = {}
    try:
        with urllib.request.urlopen(req, timeout=60) as resp:
            raw = gzip.decompress(resp.read()).decode("utf-8", "replace")
    except Exception as e:  # noqa: BLE001 — enrichment is best-effort
        print(f"  ! EPSS fetch failed: {e}", file=sys.stderr)
        return out
    for line in raw.splitlines():
        ...
        if len(parts) < 3:
            continue
        try:
            out[parts[0]] = {"score": float(parts[1]), "percentile": float(parts[2])}
        except ValueError:
            continue
    return out
...
def fetch_kev():
    """CISA Known Exploited Vulnerabilities catalog -> {cve: dateAdded}."""
    data = http_get_json(KEV_URL)
    out = {}
    if not data:
        print("  ! CISA KEV fetch failed or empty", file=sys.stderr)
        return out
...
def main():
    ...
    # Enrich with EPSS (exploitation probability) and CISA KEV (confirmed
    # exploited in the wild). Both are best-effort: a fetch failure just leaves
    # the fields null.
    epss_map = fetch_epss()
    kev_map = fetch_kev()
    kev_hits = 0
    for c in all_cves:
        ...
        added = kev_map.get(c["id"])
        c["kev"] = added or None
        if added is not None:
            kev_hits += 1
    print(f"EPSS scores loaded: {len(epss_map)} · CISA KEV catalog: {len(kev_map)} "
          f"({kev_hits} match CVEs in window)")
